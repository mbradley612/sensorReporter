#   
#
#    This class  reads the angles from the acceleromter, gyrscope
#    and mangnetometeron a BerryIMU connected to a Raspberry Pi.
#
#    To obtain a reading, put R onto the requestQueue. This will cause
#    the class to put a python object onto the responseQueue with the current
#    readings. To shutdown the class, put a Q (for quit) onto the responseQueue. This
#    will cause the class to end its loop and put C (for closed) onto the responseQueue.
#
#    This program includes two filters (low pass and mdeian) to improve the 
#    values returned from BerryIMU by reducing noise.
#
#    It is a modification of berryIMU.py from https://github.com/ozzmaker/BerryIMU/tree/master/python-BerryIMU-gryo-accel-compass-filters
#
#    http://ozzmaker.com/
...
def class BerryIMU():
	...
	def init(self):
		...
		# need to do a search and replace to add this. to these variables
	...
	# see if there is anything on the request queue.
	# if Q, set isRunning=False and write S to the response queue at the end of the loop.
	# if R, create a python object with the output below and write to the response queue.
